...
describe('Проверяем бургер конструктор', () => {
  ...
  describe('Проверяем добавление ингредиентов', () => {
    it('Добавляем ингредиент в конструктор', () => {
      cy.get(constructor).should('not.contain.text', bun);
      cy.get(ingredient)
        .contains(bun)
        .parent()
        .find('button')
        .click();
      cy.get(constructor).should('contain.text', bun);
    });

    it('Открывает и закрываем модалку', () => {
      cy.get(modal).should('not.exist');
      cy.get(ingredient).contains(bun).click();
      cy.get(modal).should('exist');
      cy.get(modal).should('contain', bun);
